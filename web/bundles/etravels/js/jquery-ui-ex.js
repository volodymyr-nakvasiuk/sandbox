...
$.widget( "ui.autocomplete", {
			...
			keydown: function( event ) {
				...
				case keyCode.NUMPAD_ENTER:
					...
					if ( this.menu.active ) {
						...
						suppressKeyPress = true;
						event.preventDefault();
						//замена ID города в скрытом блоке
						var cityId = this.menu.element.find('a.ui-state-focus').next('span').text();
						var targetItem = this.menu.element.find('a.ui-state-focus').next('span').attr('target');
					//	alert(cityId);
					//	$('input#cityId_'+targetItem).val(cityId);
						$('input#cityId_'+targetItem).attr('value', cityId);
	...
	_initSource: function() {
		...
		if ( $.isArray(this.options.source) ) {
			...
			this.source = function( request, response ) {
				...
				if(lang != 'en') {
					request.term = correctStr(request.term);
				}
	...
	var target = this.element.attr('id');
		return $( "<li>" )
			.append( $( "<a>" ).text( item.label ) ).append('<span target="'+target+'" style="display:none;">'+item.id+'</span>')
			.appendTo( ul );
	},
